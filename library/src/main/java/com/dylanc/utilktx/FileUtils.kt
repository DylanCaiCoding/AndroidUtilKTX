...
inline fun isFileDir(filePath: String): Boolean = FileUtils.isDir(filePath)
...
inline fun createFileOrExistsDir(filePath: String): Boolean = FileUtils.createOrExistsDir(filePath)

inline fun File.createOrExistsDir(): Boolean = FileUtils.createOrExistsDir(this)

inline fun createFileOrExistsFile(filePath: String): Boolean = FileUtils.createOrExistsFile(filePath)
...
inline fun deleteFile(srcPath: String): Boolean = FileUtils.delete(srcPath)

inline fun File.delete(deleteDir: Boolean = true): Boolean =
  if (deleteDir) FileUtils.delete(this) else delete()

inline fun deleteAllFileInDir(srcPath: String): Boolean = FileUtils.deleteAllInDir(srcPath)

inline fun File.deleteAllInDir(): Boolean = FileUtils.deleteAllInDir(this)

inline fun deleteFilesInDir(srcPath: String): Boolean = FileUtils.deleteFilesInDir(srcPath)

inline fun File.deleteFilesInDir(): Boolean = FileUtils.deleteFilesInDir(this)

inline fun deleteFilesInDirWithFilter(srcPath: String, noinline filter: (File) -> Boolean): Boolean =
...
inline fun filesOf(dirPath: String, isRecursive: Boolean, noinline comparator: (File, File) -> Int): List<File> =
  FileUtils.listFilesInDir(dirPath, isRecursive, comparator)

inline fun File.filesOf(isRecursive: Boolean, noinline comparator: (File, File) -> Int): List<File> =
...
inline fun fileMD5StringOf(filePath: String): String = FileUtils.getFileMD5ToString(filePath)

inline val File.MD5String: String get() = FileUtils.getFileMD5ToString(this)
